...
def advent_of_code_2020_6_1(in_str: str) -> None:
    """"""
    total = 0
    array_group = [0] * 26
    ...
    for i in range(input_len):
        o = ord(in_str[i])
        if o != 10:                                                         # if char is a-z
            array_group[o - 97] = 1                                         # set letter to found
        if i == input_len - 1 or (o == 10 and ord(in_str[i + 1]) == 10):    # if input end or current and next char \n
            for j in range(len(array_group)):                               # loop through letters
                total += array_group[j]                                     # add found letter count to total
            array_group = [0] * 26                                          # reset letter array
    print(total)


def advent_of_code_2020_6_2(in_str: str) -> None:
    """"""
    total = 0
    array_group = [0] * 26
    input_len = len(in_str)
    ...
    for i in range(input_len):
        o = ord(in_str[i])
        if o != 10:                                                         # if char is a-z
            array_group[o - 97] += 1                                        # inc found char count
            if i == input_len - 1 or ord(in_str[i + 1]) == 10:              # if input end or following char \n
                member_count += 1                                           # inc group member count
        if i == input_len - 1 or (o == 10 and ord(in_str[i + 1]) == 10):    # if input end or current and next char \n
            for j in range(len(array_group)):                               # loop through letters
                if array_group[j] == member_count:                          # if letter found count equals group member count
                    total += 1                                              # inc total
            array_group = [0] * len(array_group)                            # reset letter array
            member_count = 0                                                # reset group member count
    print(total)


if __name__ == '__main__':
    advent_of_code_2020_6_1(in_str=input_string)
    advent_of_code_2020_6_2(in_str=input_string)
