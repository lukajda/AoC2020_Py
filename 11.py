...
def advent_of_code_2020_11_1(il: List[str]) -> None:
    """"""
    len_row = len(il)
    len_col = len(il[0])
    nl = [[ord(il[i][j]) for j in range(len_col)] for i in range(len_row)]                                                                                  # convert sting array to ord int array
    while True:                                                                                                                                             # repeat until start and changed array are equal
        nl1 = []
        for i in range(len_row):
            nl1.append([])
            for j in range(len_col):
                if nl[i][j] == DOT:                                                                                                                         # if floor copy and continue
                    nl1[i].append(DOT)
                else:
                    e = f = 0                                                                                                                               # reset empty and full counters
                    for m, n in [(m, n) for m in (i-1, i, i+1) for n in (j-1, j, j+1) if (0 <= m < len_row and 0 <= n < len_col) and (m != i or n != j)]:   # generate surrounding coordinates guarding list boundaries and excluding center
                        if nl[m][n] in (DOT, EMP):                                                                                                          # if empty seat or floor increase empty counter
                            e += 1
                        else:
                            f += 1                                                                                                                          # else increase full counter
                            if f >= 4:                                                                                                                      # if full counter >= 4 switch seat to empty and break loop
                                nl1[i].append(EMP)
                                break
                    else:                                                                                                                                   # if full surroundings search loop finished without break
                        if nl[i][j] == EMP and f == 0:                                                                                                      # if seat empty and full counter is 0 add occupied and continue
                            nl1[i].append(OCC)
                            continue
                        nl1[i].append(nl[i][j])                                                                                                             # otherwise just copy value
        if nl == nl1:                                                                                                                                       # if start and changed array are equal break
            break
        nl = nl1                                                                                                                                            # otherwise store changed array as start for next loop
    print(len([i for i in nl for j in i if j == OCC]))                                                                                                      # count occupied seats


def advent_of_code_2020_11_2(il: List[str]) -> None:
    """"""


if __name__ == '__main__':
    advent_of_code_2020_11_1(il=input_list)
    # advent_of_code_2020_11_2(il=input_list)
