input_string_list = ['wavy green bags contain 1 posh black bag, 1 faded green bag, 4 wavy red bags.',
...
def recurs1(bag_dict: Dict[str, str], search_bag: str, bag_set: set) -> None:
    """"""
    for key, val in bag_dict.items():
        if search_bag in val:
            bag_set.add(key)
            recurs1(bag_dict, key, bag_set)


def advent_of_code_2020_7_1(isl: List[str]) -> None:
    """"""
    c = re.compile(pattern='^([a-zA-Z ]+) bags contain ([0-9] ([ a-zA-Z]+)+ .* bag[s]?[.,])')   # pre-compile regex pattern to optimize
    d = {m.group(1): m.group(2) for m in [c.match(isl[i]) for i in range(len(isl))] if m}       # create dict bag to contained bags string e.g. {'wavy green': '1 posh black bag, 1 faded green bag, 4 wavy red bags.', ...}
    bag_set = set()
    recurs1(d, 'shiny gold', bag_set)                                                           # recursively iterate through dict
    print(len(bag_set))


def recurs2(bag_dict: Dict[str, Dict[str, int]], search_bag: str, count: int, mult: int) -> int:
    """"""
    count += mult
    if search_bag in bag_dict:
        for key, val in bag_dict[search_bag].items():
            count = recurs2(bag_dict, key, count, (mult * val) if mult else val)
    return count


def advent_of_code_2020_7_2(isl: List[str]) -> None:
    """"""
    c1 = re.compile(pattern='^([a-zA-Z ]+) bags contain ([0-9] ([ a-zA-Z]+)+ .* bag[s]?[.,])')                                      # pre-compile regex pattern to optimize
    c2 = re.compile(pattern='([0-9]+) ([a-zA-Z]+ [a-zA-Z]+)')                                                                       # pre-compile regex pattern to optimize
    d = {m.group(1): {l[1]: int(l[0]) for l in c2.findall(m.group(2))} for m in [c1.match(isl[i]) for i in range(len(isl))] if m}   # create dict bag to contained bags dict bag to count e.g. {'wavy green': {'posh black': 1, 'faded green': 1, 'wavy red': 4}, ...}
    count = recurs2(d, 'shiny gold', 0, 0)
    print(count)


if __name__ == '__main__':
    advent_of_code_2020_7_1(isl=input_string_list)
    advent_of_code_2020_7_2(isl=input_string_list)
